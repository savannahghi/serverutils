...
// ReadRequestToTarget handles the process of sending requests to a RESTFUL API though the supplied client and reading back the response as bytes.
...
// ReadAuthServerRequestToTarget handles the process of sending requests to a auth server URL through th supplied client and reading back the response as bytes.
// Network and IO errors are reported back.
// `target` **MUST** be a pointer to a struct - the struct to which the JSON response should be decoded
...
// PerformRequest implements the final common pathway for requests to RESTFUL APIs (regardless of the server the request is going to)
...
// ReadWriteRequestToTarget handles the process of sending requests to a RESTFUL API though the supplied client, reading back the response as bytes
...
// PerformRequestWithTarget implements the final common pathway for requests to RESTFUL APIs (regardless of the server the request is going to)

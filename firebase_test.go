...
func TestCreateFirebasePhoneNumberAuthToken(t *testing.T) {
	ctx := context.Background()
	type args struct {
		ctx         context.Context
		phoneNumber string
	...
	tests := []struct {
		...
		{
			name: "create or get phone number auth token",
			args: args{
				ctx:         ctx,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := base.CreateFirebasePhoneNumberAuthToken(tt.args.ctx, tt.args.phoneNumber)
			assert.NotNil(t, got)
			if (err != nil) != tt.wantErr {
				t.Errorf("GetPhoneNumberAuthToken() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
		})
	}

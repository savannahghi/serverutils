...
func TestValidatePhoneNumberLoginCreds(t *testing.T) {
	goodCreds := &base.PhoneNumberLoginCreds{
	...
	tests := []struct {
		name    string
		args    args
		want    *base.PhoneNumberLoginCreds
		wantErr bool
	}{
		{
			name: "valid creds",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := base.ValidatePhoneNumberLoginCreds(tt.args.w, tt.args.r)
			if (err != nil) != tt.wantErr {
				t.Errorf("ValidateLoginCreds() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
...
func TestGetPhoneNumberLoginFunc(t *testing.T) {
	ctx := context.Background()
	fc := &base.FirebaseClient{}
	loginFunc := base.GetPhoneNumberLoginFunc(ctx, fc)

	goodLoginCredsJSONBytes, err := json.Marshal(&base.PhoneNumberLoginCreds{
		PhoneNumber: base.TestUserPhoneNumberWithPin,
		Pin:         base.TestUserPin,
	})
	assert.Nil(t, err)
	...
	incorrectLoginCredsJSONBytes, err := json.Marshal(&base.PhoneNumberLoginCreds{
	...
	badLoginCredsJSONBytes, err := json.Marshal(&base.PhoneNumberLoginCreds{
		PhoneNumber: base.TestUserPhoneNumberWithPin,
		Pin:         "wrong pin number",
	})
	assert.Nil(t, err)
	assert.NotNil(t, badLoginCredsJSONBytes)
	...
	nonExistentLoginCredsJSONBytes, err := json.Marshal(&base.PhoneNumberLoginCreds{
		PhoneNumber: "+254780654321",
		Pin:         "0000",
	})
	assert.Nil(t, err)
	assert.NotNil(t, nonExistentLoginCredsJSONBytes)
	...
	noPinLoginCredsJSONBytes, err := json.Marshal(&base.PhoneNumberLoginCreds{
		PhoneNumber: "+254711223344",
		Pin:         "has no pin",
	})
	assert.Nil(t, err)
	assert.NotNil(t, noPinLoginCredsJSONBytes)
